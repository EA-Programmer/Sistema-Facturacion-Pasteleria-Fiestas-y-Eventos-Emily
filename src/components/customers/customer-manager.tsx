...
const storageKey = "emily-billing-customers-v1";
...
export function CustomerManager({ initialCustomers }: { initialCustomers: BillingCustomer[] }) {
  ...
  const [form, setForm] = useState<BillingCustomer>(emptyForm);
  ...
  function editCustomer(customer: BillingCustomer) {
        ...
        >
          <div className="border-b border-[var(--line)] p-5">
            ...
            <p className="mt-1 text-sm text-slate-500">
              Datos de facturacion y contacto para preparar pedidos.
            </p>
          </div>
          ...
          <form className="space-y-4 p-5" onSubmit={(event) => event.preventDefault()}>
            ...
            <div className="flex flex-col gap-2 sm:flex-row">
              <Button className="flex-1" disabled={isPending} onClick={saveCustomer}>
                {editingId ? <FileText aria-hidden className="size-4" /> : <Plus aria-hidden className="size-4" />}
                {isPending ? "Guardando..." : editingId ? "Guardar cambios" : "Guardar cliente"}
              </Button>
              ...
              <div className="p-5" key={customer.id}>
                <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
                  ...
                  <div className="flex shrink-0 flex-wrap gap-2">
                    ...
                    <IconButton disabled={isPending} label="Editar cliente" onClick={() => editCustomer(customer)}>
                      <Pencil aria-hidden className="size-4" />
                    </IconButton>
                    <IconButton disabled={isPending} label={customer.active ? "Desactivar cliente" : "Activar cliente"} onClick={() => toggleCustomer(customer.id)}>
                      {customer.active ? <X aria-hidden className="size-4" /> : <Check aria-hidden className="size-4" />}
                    </IconButton>
                    <IconButton disabled={isPending} label="Eliminar cliente" onClick={() => removeCustomer(customer.id)}>
