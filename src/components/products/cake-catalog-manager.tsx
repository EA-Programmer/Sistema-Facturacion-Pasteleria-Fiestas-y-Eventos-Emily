...
function GeneralProductsCrud({
  ...
  const [form, setForm] = useState<GeneralProduct>({
    id: "",
    name: "",
    description: "",
  ...
  function saveProduct() {
    ...
    if (!payload.name) {
      setMessage({ type: "error", text: "Ingresa el nombre del producto." });
      return;
    }

    if (!Number.isFinite(payload.basePrice) || payload.basePrice < 0) {
      setMessage({ type: "error", text: "El precio debe ser mayor o igual a cero." });
      return;
    }
  ...
  return (
    ...
    >
      <div className="space-y-4">
        ...
        <div className="grid gap-3 lg:grid-cols-[1.2fr_1fr_150px_170px_auto]">
          <input
          ...
          />
          <select
            className="rounded-lg border border-[var(--line)] bg-white px-3 py-2.5 text-sm outline-none focus:border-[var(--berry)]"
            onChange={(event) =>
              setForm((current) => ({
                ...current,
        ...
        <textarea
          className="min-h-20 w-full resize-y rounded-lg border border-[var(--line)] px-3 py-2.5 text-sm outline-none focus:border-[var(--berry)]"
          onChange={(event) => setForm((current) => ({ ...current, description: event.target.value }))}
          placeholder="Descripcion opcional: unidad, sabores, presentacion o notas internas."
      ...
      <div className="mt-5 divide-y divide-[var(--line)]">
        {items.length ? (
          items.map((product) => (
            <div className="flex flex-col gap-3 py-3 xl:flex-row xl:items-center xl:justify-between" key={product.id}>
              ...
              <div className="flex items-center gap-2">
                <StatusButton active={product.active} onClick={() => toggleProduct(product)} />
                <IconButton label="Editar producto" onClick={() => editProduct(product)}>
                  <Pencil aria-hidden className="size-4" />
                </IconButton>
                <IconButton label="Eliminar producto" onClick={() => removeProduct(product.id)}>
